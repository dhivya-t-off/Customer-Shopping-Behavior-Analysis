...
-- the average purchase amount between standard and express shipping
...
--the average spend and total revenue between subscribes and non-subscribes
...
select item_purchased, 100*sum(case when discount_applied = 'Yes'
then 1 else 0 end)/ count(*) * 100 as discount_rate
from customer
group by item_purchased
...
select subscription_status, count(customer_id) as repeat_layers
from customer
